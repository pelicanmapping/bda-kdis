...
void Connection::bindSocket()
{
    ...
    // JB:  Bind to the specified interface if it's set
    Address.sin_addr.s_addr = m_sBindInterface.empty()? htonl( INADDR_ANY ) : inet_addr( m_sBindInterface.c_str() );
...
Connection::Connection( const KString & SendAddress,  KUINT32 Port /* = 3000 */, KBOOL SendAddressIsMulticast /* = false */, const KString & BindInterface /* = ""*/, 
                         int TTL /*= -1*/, KBOOL DisableLoopBack /*= false*/, KBOOL Blocking /* = true */, PDU_Factory * Custom /* = 0 */ ) :
    m_uiPort( Port ),
    m_bBlockingSocket( Blocking ),
    m_sBindInterface( BindInterface ),
    m_iTTL( TTL ),
    m_bDisableLoopBack( DisableLoopBack )
{
...
void Connection::AddMulticastAddress( const KString & A ) throw( KException )
{
    ...
    ip_mreq mc;
    mc.imr_multiaddr.s_addr = inet_addr( A.c_str() );

    //

    // JB:  Bind to the specified interface if it's set
    mc.imr_interface.s_addr = m_sBindInterface.empty()? htonl( INADDR_ANY ) : inet_addr( m_sBindInterface.c_str() );
    ...
    if (m_iTTL > 0)
    {        
        unsigned char ttl = m_iTTL;
        iRet = setsockopt(m_iSocket[RECEIVE_SOCK], IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl));
        if( iRet == SOCKET_ERROR )
        {
            THROW_ERROR;
        }
    ...
    // JB: Disable loopback if desired    
    if (m_bDisableLoopBack)
    {
        unsigned char loopBack = 0;
        iRet = setsockopt(m_iSocket[RECEIVE_SOCK], IPPROTO_IP, IP_MULTICAST_LOOP, &loopBack, sizeof(loopBack));
    }
}
